...
| M6-4 | **Actual TPU inference** proven (inference_speed drifts off the init default)? | **YES — PROVEN on the 2026-07-08 camera-return run**: `inference_speed=8.61ms`, drifted off the exact `10.0` init default (D2) — real frigate-driven TPU inferences. In the 2026-07-07 authoritative run this was SKIP (deferred, not failed): the camera was off-network and the looping testclip cannot drive the detector (D6), so no frame source existed; the livecam-gated MONEY check SKIPped with reason while the coral-pid liveness assertion (M6-2) hard-asserted. The deferred check auto-asserted on the camera-return run exactly as designed — zero harness changes. | m6-camera-return-run.txt (`PASS: coral phase MONEY: coral detector alive + inference_speed moved off 10.0 init default`; `coral finding: inference_speed=8.61ms (init default 10.0 exactly; drift = real TPU inferences)`). Deferred-run record: m6-final-run.txt (`coral finding: inference_speed=10.0ms (10.0 = init default; drift unprovable without livecam)`) |
| M6-5 | A detection **event** attributable to the Coral? | **YES — PROVEN on the 2026-07-08 camera-return run**: person event on the livecam scored **by the Coral in the coral-only pool** (unambiguous attribution — no other detector existed during the phase): label `person`, score `0.71484375`. In the 2026-07-07 authoritative run this was SKIP (deferred, not failed) — same livecam-unreachable cause as M6-4. | m6-camera-return-run.txt (`PASS: coral phase: person event detected BY THE TPU (coral-only pool)`); `spike/results/coral-phase-events.json` (`.[0].label = "person"`, `.[0].data.score = 0.71484375`, `.[0].camera = "livecam"`). Deferred-run record: m6-final-run.txt (`SKIP: coral phase: person event detected BY THE TPU — livecam-url present but camera port unreachable at harness start`) |
...
**PROVEN on the 2026-07-08 camera-return run** (the camera returned to the network the next day; single full gate, zero harness changes): actual frigate-driven TPU **inference** — `inference_speed=8.61ms`, off the 10.0 init default — and a **person event scored by the Coral** (coral-only pool, score `0.71484375`). At the time of the 2026-07-07 authoritative run both were honestly outstanding (SKIP, deferred not failed): blocked solely by the camera being off-network (`No route to host`, controller-verified — `.superpowers/sdd/task-2-report.md` Concern 1) combined with the standing fact that the looping testclip drives no detector inference (D6). The camera-return gate: **SPIKE SMOKE: ALL PASS, 114 PASS / 0 FAIL / 3 SKIP, 575 denials 0 unexpected** — the 3 remaining SKIPs are the M3 livecam money trio, scene-dependent this run (nobody in frame during *that* window; detection proven on this camera 2026-07-05 and now also by the Coral in the M6 phase window).

**Every parent-spec §4-M6 verify criterion is now met**: detection runs on the Coral; snap connections documented per accelerator.
...
| `spike/results/m6-camera-return-run.txt` | Camera-return gate (2026-07-08, camera back on network): **SPIKE SMOKE: ALL PASS**, **114 PASS / 0 FAIL / 3 SKIP**, **575 denials 0 unexpected**. Carries the M6-4/M6-5 money PASSes: `inference_speed=8.61ms` drift + `person event detected BY THE TPU (coral-only pool)`. The 3 SKIPs are the M3 livecam money trio (scene-dependent that window). Local-only. |
| `spike/results/coral-phase-journal.txt` | The two `EdgeTpuTfl.__init__` markers from the frigate unit during the phase. NOTE: per-run evidence files are overwritten by each gate run — this and the files below now hold the 2026-07-08 camera-return content; the 2026-07-07 values quoted in this doc (markers at 21:53:26→21:53:29, `frigate.frigate[459926]`) are preserved verbatim in this document and in `m6-final-run.txt` where echoed. Local-only. |
| `spike/results/coral-phase-stats.json` | `/stats` on the coral config (2026-07-08 content after overwrite; the 2026-07-07 run recorded `detectors.coral.pid = 460323`, `inference_speed = 10.0` init default, `detection_start = 0.0`). Local-only. |
...
| `spike/results/denials.txt` | Full AppArmor denial log (overwritten per run; 2026-07-07 authoritative run: 366 total, 0 unexpected; 2026-07-08 camera-return run: 575 total, 0 unexpected). Local-only. |
