...
| M5-5 | Full gate: ALL PASS? | YES — **SPIKE SMOKE: ALL PASS**. **104 PASS / 0 FAIL / 3 SKIP** (livecam-url provisioned but stream unreachable at harness start — camera scene SKIP, not a pipeline failure). AppArmor: **374 total denials, 0 unexpected**. **ZERO new allowlist arms** added across the entire M5 milestone (Task 2: "No new policy arms needed"; Task 3: "No new allowlist arm needed"; Task 4: "No new allowlist arms added"). All 374 denials are pre-existing M1–M4 arms (psutil ptrace/cmdline, nginx setgid/setuid, OpenVINO caps, Coral net_admin, etc.). Single gate run, no re-run needed. | spike/results/m5-final-run.txt (authoritative: `SPIKE SMOKE: ALL PASS`; `== denials: 374 total, 0 unexpected ==`; run start 2026-07-07T00:38:04+02:00, HEAD `74b7f0e`); task-2-report.md, task-3-report.md, task-4-report.md (no-new-arm statements per task) |
...
| PID-file gate in `certsync-run` (suppress cold-start drift error log) | M5 certsync startup race story |
...
| File | Content |
|---|---|
| `spike/results/m5-final-run.txt` | Final M5 harness run (2026-07-07T00:38:04+02:00, HEAD `74b7f0e`): **SPIKE SMOKE: ALL PASS**, **104 PASS / 0 FAIL / 3 SKIP**, 374 denials 0 unexpected. Local-only. |
...
| `spike/results/ss-tln.txt` | `ss -tlnp` capture: `:5000` loopback, `:5001` loopback, `:8971` all-interfaces. Local-only. |
