...
| B1 | Python 3.11.9 builds on core26 toolchain? | YES — GCC 15.2.0, zero errors, ~1 m 22 s | spike/results/spike-results/runtime.json |
| B2 | upstream cp311 wheels import? | 6/6 ok | spike/results/spike-results/imports.json |
...
| C3 | custom-device reaches /dev/accel on classic? | Branch (d) — open OK; self-slot connect OK, open(/dev/accel/accel0) OK; advisory CAP_SYS_ADMIN denial non-blocking; Store approval needed only for distribution | spike/results/npu-connect.txt, spike/results/spike-results/npu.json |
...
- **device interface matrix for the real snapcraft.yaml:**
  ...
  - OpenVINO GPU inference: same as VA-API, plus stage `intel-opencl-icd` and wire `/etc/OpenCL` + `/usr/lib/x86_64-linux-gnu/intel-opencl` layouts. Not turn-key from the gpu extension alone.
...
- `spike/results/media-layout-pack-error.txt` — RECORDED: pack-time rejection "defines a new top-level directory /media" (Task 6)
- `spike/results/media-layout-install.txt` — RECORDED: pack-time rejection (same artifact, original filename from Task 6 capture)
