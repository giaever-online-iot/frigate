...
printf 'OK' > frigate_v1_arm64.snap
printf 'OK' > frigate_v1_armhf.snap
case "$mode" in
  flaky)
    if [ "\$recover" -eq 1 ]; then printf 'OK' > frigate_v1_amd64.snap
    else printf 'CORRUPT' > frigate_v1_amd64.snap; exit 1; fi ;;
  partial)
    exit 1 ;;
esac
EOF
